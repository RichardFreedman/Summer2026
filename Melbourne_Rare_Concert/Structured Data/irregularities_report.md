...
- **Condensed vs. itemised.** Per your instruction, repetitive season-brochure programmes (chiefly the Musashino Academia Musicae seasons, the Michigan May Festivals, and the Pacific Contemporary Music Festival) list repertoire compactly in each section's `additional_information` with `performances: []`. Rich single programmes — the Liedertafel concerts, Cross Talk, RCM, Melba, Beecham, and similar — are fully itemised as `Performance` objects.
